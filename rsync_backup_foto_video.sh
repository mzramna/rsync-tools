...
sudo find $source -regex $regex_image_video -type f -exec sudo rsync -bairu --compress-level=9 {} $dest  \;
...
## copia todos os formatos de foto e video descobertos até o momento , da pasta origem e seus subdiretórios,para a pasta destino,COM HIERARQUIA DE DIRETÓRIOS
 sudo rsync -airu --compress-level=9 --files-from=<(sudo find $source -regex $regex_image_video -type f -exec echo {} \;) / $dest  \;
